...
public class ScoreGathering {
	public String scoreGather(List<Integer> stream){
		...
		for(Integer i: stream){
			root = insertIntoTree(root, (int)i);
		}
		return serializeTree(root);
	}
	
	private TreeNode insertIntoTree(TreeNode root, int target){
		if(root==null) return new TreeNode(target, 1);
		if(target > root.val){
			root.right = insertIntoTree(root.right,target);
		}else if(target < root.val){
			root.left = insertIntoTree(root.left, target);
		}else{
			root.freq =+1;
		}
		return root;
